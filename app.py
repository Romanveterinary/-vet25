...
# Спроба імпортувати бібліотеку Google Cloud
try:
...
def save_file_photo(file_stream, content_type, original_filename, animal_species=None, organ_type=None):
    if bucket:
        try:
            ...
            print(f"Помилка завантаження фото в GCS: {e}")
            raise
    else:
...
def download_report(period):
    try:
        if period == 'week': days, period_name = 7, "тиждень"
        elif period == 'month': days, period_name = 30, "місяць"
        else: flash('Неправильний період для звіту.', 'danger'); return redirect(url_for('admin.reports'))
        end_date = datetime.datetime.now(datetime.UTC)
        start_date = end_date - datetime.timedelta(days=days)
        users = User.query.filter(User.role != 'admin').all()
        workbook = openpyxl.Workbook()
        sheet = workbook.active
        sheet.title = f"Звіт за {period_name}"
        sheet.append(["Користувач", "Підконтрольні підприємства", "Завантажено фото", "Залишено коментарів"])
        header_font = openpyxl.styles.Font(bold=True)
        for cell in sheet[1]: cell.font = header_font
        for user in users:
            photo_count = user.photos.filter(Photo.upload_date >= start_date).count()
            comment_count = user.comments.filter(Comment.timestamp >= start_date).count()
            enterprises_str = ", ".join([e.name for e in user.enterprises]) or "Немає"
            sheet.append([user.username, enterprises_str, photo_count, comment_count])
        virtual_workbook = io.BytesIO()
        workbook.save(virtual_workbook)
        virtual_workbook.seek(0)
        filename = f'report_{period}_{datetime.date.today()}.xlsx'
        ...
        with zipfile.ZipFile(zip_buffer, 'a', zipfile.ZIP_DEFLATED, False) as zip_file:
                ...
                if not photo.filepath.startswith('http'):
                    try:
                        ...
                        if os.path.exists(full_path):
                            arcname = os.path.join('photos', str(photo.user_id), photo.filename)
            ...
            for comment in comments:
                comments_data.append({
                    'comment_id': comment.id, 'photo_id': comment.photo_id,
                    'author_username': comment.author.username, 'text': comment.text,
                    'timestamp_utc': comment.timestamp.isoformat()
                })
            
            json_data = json.dumps(comments_data, indent=4, ensure_ascii=False)
            zip_file.writestr('comments.json', json_data)
